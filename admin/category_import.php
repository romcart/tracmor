 ...
 */
    ...
    protected $lblImportLocations;
        ...
        $file = fopen(sprintf("%s/%s_category_skipped.csv", __TRACMOR_TMP__, $_SESSION['intUserAccountId']), "r");
        ob_end_clean();
      ...
      switch ($this->lstTextDelimiter->SelectedValue) {
		...
		protected function btnNext_Click() {
          ...
          // File Not Uploaded
            ...
            while ($row = fgets($file, 1000)) {
              if ($j == 1) {
                $strFilePath = sprintf('%s/%s_cat_%s.csv', __TRACMOR_TMP__, $_SESSION['intUserAccountId'], $i);
            ...
            $this->intTotalCount = ($i-1)*200 + $j-1;
              ...
              $file_skipped = fopen($this->strFilePath = sprintf('%s/%s_category_skipped.csv', __TRACMOR_TMP__, $_SESSION['intUserAccountId']), "w+");
              // Get Headers
              if ($this->blnHeaderRow) {
        ...
        if (!$blnError /*&& $blnAssetCode && $blnAssetModelCode && $blnAssetModelShortDescription && $blnLocation*/ && $blnCategory /*&& $blnManufacturer*/ && ($this->lstImportAction->SelectedValue != 2 || $blnCategoryId)) {
          ...
          $blnError = true;
		    ...
		    // Step 3 complete
		    set_time_limit(0);
		    $file_skipped = fopen($strFilePath = sprintf('%s/%s_category_skipped.csv', __TRACMOR_TMP__, $_SESSION['intUserAccountId']), "a");
    ...
    protected function in_array_nocase($search, &$array) {
	  ...
	  protected function lstMapHeader_Create($objParentObject, $intId, $strName = null) {
	    ...
	    $lstMapHeader->AddItem("Category Name", "Category Name", ($strName == 'category name') ? true : false, $strCategoryGroup, 'CssClass="redtext"');
	    $lstMapHeader->AddItem("Description", "Category Description", ($strName == 'description') ? true : false, $strCategoryGroup);
