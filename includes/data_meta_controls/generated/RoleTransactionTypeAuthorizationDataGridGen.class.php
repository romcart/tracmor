<?php
	 ...
	 * listing a collection of RoleTransactionTypeAuthorization objects.  It includes
	 ...
	 * 
	 */
	class RoleTransactionTypeAuthorizationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this RoleTransactionTypeAuthorization datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the RoleTransactionTypeAuthorization object, or it can be any QQNode extending from QQN::RoleTransactionTypeAuthorization().
		 * 
		 ...
		 * sorting using two fields from the RoleTransactionTypeAuthorization object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from RoleTransactionTypeAuthorization
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intRoleTransactionTypeAuthorizationId=<?=urlencode($_ITEM->RoleTransactionTypeAuthorizationId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->RoleTransactionTypeAuthorizationId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->RoleTransactionTypeAuthorizationId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = RoleTransactionTypeAuthorization::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from RoleTransactionTypeAuthorization, given the clauses above
			$this->DataSource = RoleTransactionTypeAuthorization::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a RoleTransactionTypeAuthorization-based QQNode.
		 * It will also verify that it is a proper RoleTransactionTypeAuthorization-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'role_transaction_type_authorization') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "role_transaction_type_authorization".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'RoleTransactionTypeAuthorizationId': return QQN::RoleTransactionTypeAuthorization()->RoleTransactionTypeAuthorizationId;
				case 'RoleId': return QQN::RoleTransactionTypeAuthorization()->RoleId;
				case 'Role': return QQN::RoleTransactionTypeAuthorization()->Role;
				case 'TransactionTypeId': return QQN::RoleTransactionTypeAuthorization()->TransactionTypeId;
				case 'TransactionType': return QQN::RoleTransactionTypeAuthorization()->TransactionType;
				case 'AuthorizationLevelId': return QQN::RoleTransactionTypeAuthorization()->AuthorizationLevelId;
				case 'AuthorizationLevel': return QQN::RoleTransactionTypeAuthorization()->AuthorizationLevel;
				case 'CreatedBy': return QQN::RoleTransactionTypeAuthorization()->CreatedBy;
				case 'CreatedByObject': return QQN::RoleTransactionTypeAuthorization()->CreatedByObject;
				case 'CreationDate': return QQN::RoleTransactionTypeAuthorization()->CreationDate;
				case 'ModifiedBy': return QQN::RoleTransactionTypeAuthorization()->ModifiedBy;
				case 'ModifiedByObject': return QQN::RoleTransactionTypeAuthorization()->ModifiedByObject;
				case 'ModifiedDate': return QQN::RoleTransactionTypeAuthorization()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in RoleTransactionTypeAuthorizationDataGrid content: ' . $mixContent);
