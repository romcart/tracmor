<?php
	 ...
	 * listing a collection of AdminSetting objects.  It includes
	 ...
	 * 
	 */
	class AdminSettingDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this AdminSetting datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the AdminSetting object, or it can be any QQNode extending from QQN::AdminSetting().
		 * 
		 ...
		 * sorting using two fields from the AdminSetting object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from AdminSetting
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intSettingId=<?=urlencode($_ITEM->SettingId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->SettingId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->SettingId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = AdminSetting::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a AdminSetting-based QQNode.
		 * It will also verify that it is a proper AdminSetting-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'admin_setting') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "admin_setting".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'SettingId': return QQN::AdminSetting()->SettingId;
				case 'ShortDescription': return QQN::AdminSetting()->ShortDescription;
				case 'Value': return QQN::AdminSetting()->Value;
				default: throw new QCallerException('Simple Property not found in AdminSettingDataGrid content: ' . $mixContent);
