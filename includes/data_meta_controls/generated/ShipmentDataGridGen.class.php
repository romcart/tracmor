<?php
	 ...
	 * listing a collection of Shipment objects.  It includes
	 ...
	 * 
	 */
	class ShipmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Shipment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Shipment object, or it can be any QQNode extending from QQN::Shipment().
		 * 
		 ...
		 * sorting using two fields from the Shipment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Shipment
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intShipmentId=<?=urlencode($_ITEM->ShipmentId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->ShipmentId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ShipmentId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Shipment::QueryCount($objCondition, $objClauses);
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'ShipmentId': return QQN::Shipment()->ShipmentId;
				case 'ShipmentNumber': return QQN::Shipment()->ShipmentNumber;
				case 'TransactionId': return QQN::Shipment()->TransactionId;
				case 'Transaction': return QQN::Shipment()->Transaction;
				case 'FromCompanyId': return QQN::Shipment()->FromCompanyId;
				case 'FromCompany': return QQN::Shipment()->FromCompany;
				case 'FromContactId': return QQN::Shipment()->FromContactId;
				case 'FromContact': return QQN::Shipment()->FromContact;
				case 'FromAddressId': return QQN::Shipment()->FromAddressId;
				case 'FromAddress': return QQN::Shipment()->FromAddress;
				case 'ToCompanyId': return QQN::Shipment()->ToCompanyId;
				case 'ToCompany': return QQN::Shipment()->ToCompany;
				case 'ToContactId': return QQN::Shipment()->ToContactId;
				case 'ToContact': return QQN::Shipment()->ToContact;
				case 'ToAddressId': return QQN::Shipment()->ToAddressId;
				case 'ToAddress': return QQN::Shipment()->ToAddress;
				case 'CourierId': return QQN::Shipment()->CourierId;
				case 'Courier': return QQN::Shipment()->Courier;
				case 'TrackingNumber': return QQN::Shipment()->TrackingNumber;
				case 'ShipDate': return QQN::Shipment()->ShipDate;
				case 'ShippedFlag': return QQN::Shipment()->ShippedFlag;
				case 'CreatedBy': return QQN::Shipment()->CreatedBy;
				case 'CreatedByObject': return QQN::Shipment()->CreatedByObject;
				case 'CreationDate': return QQN::Shipment()->CreationDate;
				case 'ModifiedBy': return QQN::Shipment()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Shipment()->ModifiedByObject;
				case 'ModifiedDate': return QQN::Shipment()->ModifiedDate;
				case 'ShipmentCustomFieldHelper': return QQN::Shipment()->ShipmentCustomFieldHelper;

				default: throw new QCallerException('Simple Property not found in ShipmentDataGrid content: ' . $mixContent);
