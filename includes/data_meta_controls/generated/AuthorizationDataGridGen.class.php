<?php
	 ...
	 * listing a collection of Authorization objects.  It includes
	 ...
	 * 
	 */
	class AuthorizationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Authorization datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Authorization object, or it can be any QQNode extending from QQN::Authorization().
		 * 
		 ...
		 * sorting using two fields from the Authorization object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Authorization
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAuthorizationId=<?=urlencode($_ITEM->AuthorizationId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AuthorizationId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AuthorizationId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Authorization::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Authorization, given the clauses above
			$this->DataSource = Authorization::QueryArray($objCondition, $objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'AuthorizationId': return QQN::Authorization()->AuthorizationId;
				case 'ShortDescription': return QQN::Authorization()->ShortDescription;
				default: throw new QCallerException('Simple Property not found in AuthorizationDataGrid content: ' . $mixContent);
