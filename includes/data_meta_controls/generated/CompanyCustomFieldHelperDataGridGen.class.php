<?php
	 ...
	 * listing a collection of CompanyCustomFieldHelper objects.  It includes
	 ...
	 * 
	 */
	class CompanyCustomFieldHelperDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CompanyCustomFieldHelper datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CompanyCustomFieldHelper object, or it can be any QQNode extending from QQN::CompanyCustomFieldHelper().
		 * 
		 ...
		 * sorting using two fields from the CompanyCustomFieldHelper object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CompanyCustomFieldHelper
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intCompanyId=<?=urlencode($_ITEM->CompanyId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->CompanyId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->CompanyId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CompanyCustomFieldHelper::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CompanyCustomFieldHelper-based QQNode.
		 * It will also verify that it is a proper CompanyCustomFieldHelper-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'company_custom_field_helper') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "company_custom_field_helper".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'CompanyId': return QQN::CompanyCustomFieldHelper()->CompanyId;
				case 'Company': return QQN::CompanyCustomFieldHelper()->Company;
				default: throw new QCallerException('Simple Property not found in CompanyCustomFieldHelperDataGrid content: ' . $mixContent);
