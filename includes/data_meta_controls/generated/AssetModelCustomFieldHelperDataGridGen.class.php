<?php
	 ...
	 * listing a collection of AssetModelCustomFieldHelper objects.  It includes
	 ...
	 * 
	 */
	class AssetModelCustomFieldHelperDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this AssetModelCustomFieldHelper datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the AssetModelCustomFieldHelper object, or it can be any QQNode extending from QQN::AssetModelCustomFieldHelper().
		 * 
		 ...
		 * sorting using two fields from the AssetModelCustomFieldHelper object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from AssetModelCustomFieldHelper
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAssetModelId=<?=urlencode($_ITEM->AssetModelId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AssetModelId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AssetModelId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = AssetModelCustomFieldHelper::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from AssetModelCustomFieldHelper, given the clauses above
			$this->DataSource = AssetModelCustomFieldHelper::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a AssetModelCustomFieldHelper-based QQNode.
		 * It will also verify that it is a proper AssetModelCustomFieldHelper-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'asset_model_custom_field_helper') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "asset_model_custom_field_helper".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'AssetModelId': return QQN::AssetModelCustomFieldHelper()->AssetModelId;
				case 'AssetModel': return QQN::AssetModelCustomFieldHelper()->AssetModel;
				default: throw new QCallerException('Simple Property not found in AssetModelCustomFieldHelperDataGrid content: ' . $mixContent);
