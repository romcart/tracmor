<?php
	 ...
	 * listing a collection of Transaction objects.  It includes
	 ...
	 * 
	 */
	class TransactionDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Transaction datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Transaction object, or it can be any QQNode extending from QQN::Transaction().
		 * 
		 ...
		 * sorting using two fields from the Transaction object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Transaction
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intTransactionId=<?=urlencode($_ITEM->TransactionId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->TransactionId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->TransactionId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Transaction::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Transaction, given the clauses above
			$this->DataSource = Transaction::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Transaction-based QQNode.
		 * It will also verify that it is a proper Transaction-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'transaction') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "transaction".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'TransactionId': return QQN::Transaction()->TransactionId;
				case 'EntityQtypeId': return QQN::Transaction()->EntityQtypeId;
				case 'TransactionTypeId': return QQN::Transaction()->TransactionTypeId;
				case 'TransactionType': return QQN::Transaction()->TransactionType;
				case 'Note': return QQN::Transaction()->Note;
				case 'CreatedBy': return QQN::Transaction()->CreatedBy;
				case 'CreatedByObject': return QQN::Transaction()->CreatedByObject;
				case 'CreationDate': return QQN::Transaction()->CreationDate;
				case 'ModifiedBy': return QQN::Transaction()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Transaction()->ModifiedByObject;
				case 'ModifiedDate': return QQN::Transaction()->ModifiedDate;
				case 'Receipt': return QQN::Transaction()->Receipt;

				case 'Shipment': return QQN::Transaction()->Shipment;

				default: throw new QCallerException('Simple Property not found in TransactionDataGrid content: ' . $mixContent);
