<?php
	 ...
	 * listing a collection of Contact objects.  It includes
	 ...
	 * 
	 */
	class ContactDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Contact datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Contact object, or it can be any QQNode extending from QQN::Contact().
		 * 
		 ...
		 * sorting using two fields from the Contact object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Contact
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intContactId=<?=urlencode($_ITEM->ContactId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->ContactId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ContactId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Contact::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Contact, given the clauses above
			$this->DataSource = Contact::QueryArray($objCondition, $objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'ContactId': return QQN::Contact()->ContactId;
				case 'CompanyId': return QQN::Contact()->CompanyId;
				case 'Company': return QQN::Contact()->Company;
				case 'AddressId': return QQN::Contact()->AddressId;
				case 'Address': return QQN::Contact()->Address;
				case 'FirstName': return QQN::Contact()->FirstName;
				case 'LastName': return QQN::Contact()->LastName;
				case 'Title': return QQN::Contact()->Title;
				case 'Email': return QQN::Contact()->Email;
				case 'PhoneOffice': return QQN::Contact()->PhoneOffice;
				case 'PhoneHome': return QQN::Contact()->PhoneHome;
				...
				case 'ModifiedBy': return QQN::Contact()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Contact()->ModifiedByObject;
				case 'ModifiedDate': return QQN::Contact()->ModifiedDate;
				case 'ContactCustomFieldHelper': return QQN::Contact()->ContactCustomFieldHelper;

				default: throw new QCallerException('Simple Property not found in ContactDataGrid content: ' . $mixContent);
