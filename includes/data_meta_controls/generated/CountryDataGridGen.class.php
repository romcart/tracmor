<?php
	 ...
	 * listing a collection of Country objects.  It includes
	 ...
	 * 
	 */
	class CountryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Country datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Country object, or it can be any QQNode extending from QQN::Country().
		 * 
		 ...
		 * sorting using two fields from the Country object.
		 *
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intCountryId=<?=urlencode($_ITEM->CountryId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->CountryId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->CountryId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Country::QueryCount($objCondition, $objClauses);
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'CountryId': return QQN::Country()->CountryId;
				case 'ShortDescription': return QQN::Country()->ShortDescription;
				case 'Abbreviation': return QQN::Country()->Abbreviation;
				case 'StateFlag': return QQN::Country()->StateFlag;
				case 'ProvinceFlag': return QQN::Country()->ProvinceFlag;
				default: throw new QCallerException('Simple Property not found in CountryDataGrid content: ' . $mixContent);
