<?php
	 ...
	 * listing a collection of InventoryLocation objects.  It includes
	 ...
	 * 
	 */
	class InventoryLocationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this InventoryLocation datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the InventoryLocation object, or it can be any QQNode extending from QQN::InventoryLocation().
		 * 
		 ...
		 * sorting using two fields from the InventoryLocation object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from InventoryLocation
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intInventoryLocationId=<?=urlencode($_ITEM->InventoryLocationId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->InventoryLocationId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->InventoryLocationId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = InventoryLocation::QueryCount($objCondition, $objClauses);
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'InventoryLocationId': return QQN::InventoryLocation()->InventoryLocationId;
				case 'InventoryModelId': return QQN::InventoryLocation()->InventoryModelId;
				case 'InventoryModel': return QQN::InventoryLocation()->InventoryModel;
				case 'LocationId': return QQN::InventoryLocation()->LocationId;
				case 'Location': return QQN::InventoryLocation()->Location;
				case 'Quantity': return QQN::InventoryLocation()->Quantity;
				case 'CreatedBy': return QQN::InventoryLocation()->CreatedBy;
				case 'CreatedByObject': return QQN::InventoryLocation()->CreatedByObject;
				case 'CreationDate': return QQN::InventoryLocation()->CreationDate;
				case 'ModifiedBy': return QQN::InventoryLocation()->ModifiedBy;
				case 'ModifiedByObject': return QQN::InventoryLocation()->ModifiedByObject;
				case 'ModifiedDate': return QQN::InventoryLocation()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in InventoryLocationDataGrid content: ' . $mixContent);
