<?php
	 ...
	 * listing a collection of DatagridColumnPreference objects.  It includes
	 ...
	 * 
	 */
	class DatagridColumnPreferenceDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this DatagridColumnPreference datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DatagridColumnPreference object, or it can be any QQNode extending from QQN::DatagridColumnPreference().
		 * 
		 ...
		 * sorting using two fields from the DatagridColumnPreference object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DatagridColumnPreference
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intDatagridColumnPreferenceId=<?=urlencode($_ITEM->DatagridColumnPreferenceId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->DatagridColumnPreferenceId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->DatagridColumnPreferenceId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = DatagridColumnPreference::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from DatagridColumnPreference, given the clauses above
			$this->DataSource = DatagridColumnPreference::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DatagridColumnPreference-based QQNode.
		 * It will also verify that it is a proper DatagridColumnPreference-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'datagrid_column_preference') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "datagrid_column_preference".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'DatagridColumnPreferenceId': return QQN::DatagridColumnPreference()->DatagridColumnPreferenceId;
				case 'DatagridId': return QQN::DatagridColumnPreference()->DatagridId;
				case 'Datagrid': return QQN::DatagridColumnPreference()->Datagrid;
				case 'ColumnName': return QQN::DatagridColumnPreference()->ColumnName;
				case 'UserAccountId': return QQN::DatagridColumnPreference()->UserAccountId;
				case 'UserAccount': return QQN::DatagridColumnPreference()->UserAccount;
				case 'DisplayFlag': return QQN::DatagridColumnPreference()->DisplayFlag;
				default: throw new QCallerException('Simple Property not found in DatagridColumnPreferenceDataGrid content: ' . $mixContent);
