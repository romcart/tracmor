<?php
	 ...
	 * listing a collection of Receipt objects.  It includes
	 ...
	 * 
	 */
	class ReceiptDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Receipt datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Receipt object, or it can be any QQNode extending from QQN::Receipt().
		 * 
		 ...
		 * sorting using two fields from the Receipt object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Receipt
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intReceiptId=<?=urlencode($_ITEM->ReceiptId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->ReceiptId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ReceiptId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Receipt::QueryCount($objCondition, $objClauses);
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'ReceiptId': return QQN::Receipt()->ReceiptId;
				case 'TransactionId': return QQN::Receipt()->TransactionId;
				case 'Transaction': return QQN::Receipt()->Transaction;
				case 'FromCompanyId': return QQN::Receipt()->FromCompanyId;
				case 'FromCompany': return QQN::Receipt()->FromCompany;
				case 'FromContactId': return QQN::Receipt()->FromContactId;
				case 'FromContact': return QQN::Receipt()->FromContact;
				case 'ToContactId': return QQN::Receipt()->ToContactId;
				case 'ToContact': return QQN::Receipt()->ToContact;
				case 'ToAddressId': return QQN::Receipt()->ToAddressId;
				case 'ToAddress': return QQN::Receipt()->ToAddress;
				case 'ReceiptNumber': return QQN::Receipt()->ReceiptNumber;
				case 'DueDate': return QQN::Receipt()->DueDate;
				case 'ReceiptDate': return QQN::Receipt()->ReceiptDate;
				case 'ReceivedFlag': return QQN::Receipt()->ReceivedFlag;
				case 'CreatedBy': return QQN::Receipt()->CreatedBy;
				case 'CreatedByObject': return QQN::Receipt()->CreatedByObject;
				case 'CreationDate': return QQN::Receipt()->CreationDate;
				case 'ModifiedBy': return QQN::Receipt()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Receipt()->ModifiedByObject;
				case 'ModifiedDate': return QQN::Receipt()->ModifiedDate;
				case 'ReceiptCustomFieldHelper': return QQN::Receipt()->ReceiptCustomFieldHelper;

				default: throw new QCallerException('Simple Property not found in ReceiptDataGrid content: ' . $mixContent);
