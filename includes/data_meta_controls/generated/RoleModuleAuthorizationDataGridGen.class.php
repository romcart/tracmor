<?php
	 ...
	 * listing a collection of RoleModuleAuthorization objects.  It includes
	 ...
	 * 
	 */
	class RoleModuleAuthorizationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this RoleModuleAuthorization datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the RoleModuleAuthorization object, or it can be any QQNode extending from QQN::RoleModuleAuthorization().
		 * 
		 ...
		 * sorting using two fields from the RoleModuleAuthorization object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from RoleModuleAuthorization
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intRoleModuleAuthorizationId=<?=urlencode($_ITEM->RoleModuleAuthorizationId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->RoleModuleAuthorizationId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->RoleModuleAuthorizationId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = RoleModuleAuthorization::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from RoleModuleAuthorization, given the clauses above
			$this->DataSource = RoleModuleAuthorization::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a RoleModuleAuthorization-based QQNode.
		 * It will also verify that it is a proper RoleModuleAuthorization-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'role_module_authorization') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "role_module_authorization".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'RoleModuleAuthorizationId': return QQN::RoleModuleAuthorization()->RoleModuleAuthorizationId;
				case 'RoleModuleId': return QQN::RoleModuleAuthorization()->RoleModuleId;
				case 'RoleModule': return QQN::RoleModuleAuthorization()->RoleModule;
				case 'AuthorizationId': return QQN::RoleModuleAuthorization()->AuthorizationId;
				case 'Authorization': return QQN::RoleModuleAuthorization()->Authorization;
				case 'AuthorizationLevelId': return QQN::RoleModuleAuthorization()->AuthorizationLevelId;
				case 'AuthorizationLevel': return QQN::RoleModuleAuthorization()->AuthorizationLevel;
				case 'CreatedBy': return QQN::RoleModuleAuthorization()->CreatedBy;
				case 'CreatedByObject': return QQN::RoleModuleAuthorization()->CreatedByObject;
				case 'CreationDate': return QQN::RoleModuleAuthorization()->CreationDate;
				case 'ModifiedBy': return QQN::RoleModuleAuthorization()->ModifiedBy;
				case 'ModifiedByObject': return QQN::RoleModuleAuthorization()->ModifiedByObject;
				case 'ModifiedDate': return QQN::RoleModuleAuthorization()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in RoleModuleAuthorizationDataGrid content: ' . $mixContent);
