<?php
	 ...
	 * listing a collection of Datagrid objects.  It includes
	 ...
	 * 
	 */
	class DatagridDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Datagrid datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Datagrid object, or it can be any QQNode extending from QQN::Datagrid().
		 * 
		 ...
		 * sorting using two fields from the Datagrid object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Datagrid
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intDatagridId=<?=urlencode($_ITEM->DatagridId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->DatagridId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->DatagridId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Datagrid::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Datagrid, given the clauses above
			$this->DataSource = Datagrid::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Datagrid-based QQNode.
		 * It will also verify that it is a proper Datagrid-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'datagrid') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "datagrid".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'DatagridId': return QQN::Datagrid()->DatagridId;
				case 'ShortDescription': return QQN::Datagrid()->ShortDescription;
				default: throw new QCallerException('Simple Property not found in DatagridDataGrid content: ' . $mixContent);
