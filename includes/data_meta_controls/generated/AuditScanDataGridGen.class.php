<?php
	 ...
	 * listing a collection of AuditScan objects.  It includes
	 ...
	 * 
	 */
	class AuditScanDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this AuditScan datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the AuditScan object, or it can be any QQNode extending from QQN::AuditScan().
		 * 
		 ...
		 * sorting using two fields from the AuditScan object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from AuditScan
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAuditScanId=<?=urlencode($_ITEM->AuditScanId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AuditScanId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AuditScanId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = AuditScan::QueryCount($objCondition, $objClauses);
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'AuditScanId': return QQN::AuditScan()->AuditScanId;
				case 'AuditId': return QQN::AuditScan()->AuditId;
				case 'Audit': return QQN::AuditScan()->Audit;
				case 'LocationId': return QQN::AuditScan()->LocationId;
				case 'Location': return QQN::AuditScan()->Location;
				case 'EntityId': return QQN::AuditScan()->EntityId;
				case 'Count': return QQN::AuditScan()->Count;
				case 'SystemCount': return QQN::AuditScan()->SystemCount;
				default: throw new QCallerException('Simple Property not found in AuditScanDataGrid content: ' . $mixContent);
