<?php
	 ...
	 * listing a collection of AssetTransactionCheckout objects.  It includes
	 ...
	 * 
	 */
	class AssetTransactionCheckoutDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this AssetTransactionCheckout datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the AssetTransactionCheckout object, or it can be any QQNode extending from QQN::AssetTransactionCheckout().
		 * 
		 ...
		 * sorting using two fields from the AssetTransactionCheckout object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from AssetTransactionCheckout
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAssetTransactionCheckoutId=<?=urlencode($_ITEM->AssetTransactionCheckoutId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AssetTransactionCheckoutId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AssetTransactionCheckoutId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = AssetTransactionCheckout::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a AssetTransactionCheckout-based QQNode.
		 * It will also verify that it is a proper AssetTransactionCheckout-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'asset_transaction_checkout') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "asset_transaction_checkout".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'AssetTransactionCheckoutId': return QQN::AssetTransactionCheckout()->AssetTransactionCheckoutId;
				case 'AssetTransactionId': return QQN::AssetTransactionCheckout()->AssetTransactionId;
				case 'AssetTransaction': return QQN::AssetTransactionCheckout()->AssetTransaction;
				case 'ToContactId': return QQN::AssetTransactionCheckout()->ToContactId;
				case 'ToContact': return QQN::AssetTransactionCheckout()->ToContact;
				case 'ToUserId': return QQN::AssetTransactionCheckout()->ToUserId;
				case 'ToUser': return QQN::AssetTransactionCheckout()->ToUser;
				case 'DueDate': return QQN::AssetTransactionCheckout()->DueDate;
				case 'CreatedBy': return QQN::AssetTransactionCheckout()->CreatedBy;
				case 'CreatedByObject': return QQN::AssetTransactionCheckout()->CreatedByObject;
				case 'CreationDate': return QQN::AssetTransactionCheckout()->CreationDate;
				case 'ModifiedBy': return QQN::AssetTransactionCheckout()->ModifiedBy;
				case 'ModifiedByObject': return QQN::AssetTransactionCheckout()->ModifiedByObject;
				case 'ModifiedDate': return QQN::AssetTransactionCheckout()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in AssetTransactionCheckoutDataGrid content: ' . $mixContent);
