<?php
	 ...
	 * listing a collection of AssetCustomFieldAssetModel objects.  It includes
	 ...
	 * 
	 */
	class AssetCustomFieldAssetModelDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this AssetCustomFieldAssetModel datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the AssetCustomFieldAssetModel object, or it can be any QQNode extending from QQN::AssetCustomFieldAssetModel().
		 * 
		 ...
		 * sorting using two fields from the AssetCustomFieldAssetModel object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from AssetCustomFieldAssetModel
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAssetCustomFieldAssetModelId=<?=urlencode($_ITEM->AssetCustomFieldAssetModelId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AssetCustomFieldAssetModelId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AssetCustomFieldAssetModelId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = AssetCustomFieldAssetModel::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from AssetCustomFieldAssetModel, given the clauses above
			$this->DataSource = AssetCustomFieldAssetModel::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a AssetCustomFieldAssetModel-based QQNode.
		 * It will also verify that it is a proper AssetCustomFieldAssetModel-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'asset_custom_field_asset_model') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "asset_custom_field_asset_model".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'AssetCustomFieldAssetModelId': return QQN::AssetCustomFieldAssetModel()->AssetCustomFieldAssetModelId;
				case 'AssetModelId': return QQN::AssetCustomFieldAssetModel()->AssetModelId;
				case 'AssetModel': return QQN::AssetCustomFieldAssetModel()->AssetModel;
				case 'CustomFieldId': return QQN::AssetCustomFieldAssetModel()->CustomFieldId;
				case 'CustomField': return QQN::AssetCustomFieldAssetModel()->CustomField;
				default: throw new QCallerException('Simple Property not found in AssetCustomFieldAssetModelDataGrid content: ' . $mixContent);
