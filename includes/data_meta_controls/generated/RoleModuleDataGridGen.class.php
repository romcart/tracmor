<?php
	 ...
	 * listing a collection of RoleModule objects.  It includes
	 ...
	 * 
	 */
	class RoleModuleDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this RoleModule datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the RoleModule object, or it can be any QQNode extending from QQN::RoleModule().
		 * 
		 ...
		 * sorting using two fields from the RoleModule object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from RoleModule
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intRoleModuleId=<?=urlencode($_ITEM->RoleModuleId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->RoleModuleId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->RoleModuleId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = RoleModule::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from RoleModule, given the clauses above
			$this->DataSource = RoleModule::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a RoleModule-based QQNode.
		 * It will also verify that it is a proper RoleModule-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'role_module') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "role_module".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'RoleModuleId': return QQN::RoleModule()->RoleModuleId;
				case 'RoleId': return QQN::RoleModule()->RoleId;
				case 'Role': return QQN::RoleModule()->Role;
				case 'ModuleId': return QQN::RoleModule()->ModuleId;
				case 'Module': return QQN::RoleModule()->Module;
				case 'AccessFlag': return QQN::RoleModule()->AccessFlag;
				case 'CreatedBy': return QQN::RoleModule()->CreatedBy;
				case 'CreatedByObject': return QQN::RoleModule()->CreatedByObject;
				case 'CreationDate': return QQN::RoleModule()->CreationDate;
				case 'ModifiedBy': return QQN::RoleModule()->ModifiedBy;
				case 'ModifiedByObject': return QQN::RoleModule()->ModifiedByObject;
				case 'ModifiedDate': return QQN::RoleModule()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in RoleModuleDataGrid content: ' . $mixContent);
