<?php
	...
	class LocationDataGridGen extends QDataGrid {
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intLocationId=<?=urlencode($_ITEM->LocationId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->LocationId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->LocationId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Location::CountAll();
			}
			...
			$this->DataSource = Location::LoadAll($objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'LocationId': return QQN::Location()->LocationId;
				case 'ShortDescription': return QQN::Location()->ShortDescription;
				case 'LongDescription': return QQN::Location()->LongDescription;
				case 'EnabledFlag': return QQN::Location()->EnabledFlag;
				case 'AssetFlag': return QQN::Location()->AssetFlag;
				case 'InventoryFlag': return QQN::Location()->InventoryFlag;
				case 'CreatedBy': return QQN::Location()->CreatedBy;
				case 'CreatedByObject': return QQN::Location()->CreatedByObject;
				case 'CreationDate': return QQN::Location()->CreationDate;
				case 'ModifiedBy': return QQN::Location()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Location()->ModifiedByObject;
				case 'ModifiedDate': return QQN::Location()->ModifiedDate;
