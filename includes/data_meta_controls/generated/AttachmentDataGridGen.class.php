<?php
	 ...
	 * listing a collection of Attachment objects.  It includes
	 ...
	 * 
	 */
	class AttachmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Attachment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Attachment object, or it can be any QQNode extending from QQN::Attachment().
		 * 
		 ...
		 * sorting using two fields from the Attachment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Attachment
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAttachmentId=<?=urlencode($_ITEM->AttachmentId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AttachmentId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AttachmentId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Attachment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Attachment, given the clauses above
			$this->DataSource = Attachment::QueryArray($objCondition, $objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'AttachmentId': return QQN::Attachment()->AttachmentId;
				case 'EntityQtypeId': return QQN::Attachment()->EntityQtypeId;
				case 'EntityId': return QQN::Attachment()->EntityId;
				case 'Filename': return QQN::Attachment()->Filename;
				case 'TmpFilename': return QQN::Attachment()->TmpFilename;
				case 'FileType': return QQN::Attachment()->FileType;
				case 'Path': return QQN::Attachment()->Path;
				case 'Size': return QQN::Attachment()->Size;
				case 'CreatedBy': return QQN::Attachment()->CreatedBy;
				case 'CreatedByObject': return QQN::Attachment()->CreatedByObject;
				case 'CreationDate': return QQN::Attachment()->CreationDate;
				default: throw new QCallerException('Simple Property not found in AttachmentDataGrid content: ' . $mixContent);
