<?php
	 ...
	 * listing a collection of AddressCustomFieldHelper objects.  It includes
	 ...
	 * 
	 */
	class AddressCustomFieldHelperDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this AddressCustomFieldHelper datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the AddressCustomFieldHelper object, or it can be any QQNode extending from QQN::AddressCustomFieldHelper().
		 * 
		 ...
		 * sorting using two fields from the AddressCustomFieldHelper object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from AddressCustomFieldHelper
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAddressId=<?=urlencode($_ITEM->AddressId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AddressId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AddressId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = AddressCustomFieldHelper::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a AddressCustomFieldHelper-based QQNode.
		 * It will also verify that it is a proper AddressCustomFieldHelper-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'address_custom_field_helper') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "address_custom_field_helper".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'AddressId': return QQN::AddressCustomFieldHelper()->AddressId;
				case 'Address': return QQN::AddressCustomFieldHelper()->Address;
				default: throw new QCallerException('Simple Property not found in AddressCustomFieldHelperDataGrid content: ' . $mixContent);
