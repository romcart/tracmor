<?php
	 ...
	 * listing a collection of CustomFieldValue objects.  It includes
	 ...
	 * 
	 */
	class CustomFieldValueDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CustomFieldValue datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CustomFieldValue object, or it can be any QQNode extending from QQN::CustomFieldValue().
		 * 
		 ...
		 * sorting using two fields from the CustomFieldValue object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CustomFieldValue
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intCustomFieldValueId=<?=urlencode($_ITEM->CustomFieldValueId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->CustomFieldValueId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->CustomFieldValueId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CustomFieldValue::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CustomFieldValue-based QQNode.
		 * It will also verify that it is a proper CustomFieldValue-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'custom_field_value') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "custom_field_value".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'CustomFieldValueId': return QQN::CustomFieldValue()->CustomFieldValueId;
				case 'CustomFieldId': return QQN::CustomFieldValue()->CustomFieldId;
				case 'CustomField': return QQN::CustomFieldValue()->CustomField;
				case 'ShortDescription': return QQN::CustomFieldValue()->ShortDescription;
				case 'CreatedBy': return QQN::CustomFieldValue()->CreatedBy;
				case 'CreatedByObject': return QQN::CustomFieldValue()->CreatedByObject;
				case 'CreationDate': return QQN::CustomFieldValue()->CreationDate;
				case 'ModifiedBy': return QQN::CustomFieldValue()->ModifiedBy;
				case 'ModifiedByObject': return QQN::CustomFieldValue()->ModifiedByObject;
				case 'ModifiedDate': return QQN::CustomFieldValue()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in CustomFieldValueDataGrid content: ' . $mixContent);
