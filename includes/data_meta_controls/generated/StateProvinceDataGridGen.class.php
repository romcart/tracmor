<?php
	 ...
	 * listing a collection of StateProvince objects.  It includes
	 ...
	 * 
	 */
	class StateProvinceDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this StateProvince datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the StateProvince object, or it can be any QQNode extending from QQN::StateProvince().
		 * 
		 ...
		 * sorting using two fields from the StateProvince object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from StateProvince
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intStateProvinceId=<?=urlencode($_ITEM->StateProvinceId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->StateProvinceId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->StateProvinceId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = StateProvince::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a StateProvince-based QQNode.
		 * It will also verify that it is a proper StateProvince-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'state_province') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "state_province".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'StateProvinceId': return QQN::StateProvince()->StateProvinceId;
				case 'CountryId': return QQN::StateProvince()->CountryId;
				case 'Country': return QQN::StateProvince()->Country;
				case 'ShortDescription': return QQN::StateProvince()->ShortDescription;
				case 'Abbreviation': return QQN::StateProvince()->Abbreviation;
				default: throw new QCallerException('Simple Property not found in StateProvinceDataGrid content: ' . $mixContent);
