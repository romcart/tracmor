<?php
	 ...
	 * listing a collection of Manufacturer objects.  It includes
	 ...
	 * 
	 */
	class ManufacturerDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Manufacturer datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Manufacturer object, or it can be any QQNode extending from QQN::Manufacturer().
		 * 
		 ...
		 * sorting using two fields from the Manufacturer object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Manufacturer
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intManufacturerId=<?=urlencode($_ITEM->ManufacturerId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->ManufacturerId))?>';
					break;
				default:
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Manufacturer::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Manufacturer, given the clauses above
			$this->DataSource = Manufacturer::QueryArray($objCondition, $objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'ManufacturerId': return QQN::Manufacturer()->ManufacturerId;
				case 'ShortDescription': return QQN::Manufacturer()->ShortDescription;
				case 'LongDescription': return QQN::Manufacturer()->LongDescription;
				case 'ImagePath': return QQN::Manufacturer()->ImagePath;
				case 'CreatedBy': return QQN::Manufacturer()->CreatedBy;
				case 'CreatedByObject': return QQN::Manufacturer()->CreatedByObject;
				case 'CreationDate': return QQN::Manufacturer()->CreationDate;
				case 'ModifiedBy': return QQN::Manufacturer()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Manufacturer()->ModifiedByObject;
				case 'ModifiedDate': return QQN::Manufacturer()->ModifiedDate;
				case 'ManufacturerCustomFieldHelper': return QQN::Manufacturer()->ManufacturerCustomFieldHelper;

				default: throw new QCallerException('Simple Property not found in ManufacturerDataGrid content: ' . $mixContent);
