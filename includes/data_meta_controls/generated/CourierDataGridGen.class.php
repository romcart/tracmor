<?php
	 ...
	 * listing a collection of Courier objects.  It includes
	 ...
	 * 
	 */
	class CourierDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Courier datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Courier object, or it can be any QQNode extending from QQN::Courier().
		 * 
		 ...
		 * sorting using two fields from the Courier object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Courier
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intCourierId=<?=urlencode($_ITEM->CourierId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->CourierId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->CourierId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Courier::QueryCount($objCondition, $objClauses);
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'CourierId': return QQN::Courier()->CourierId;
				case 'ShortDescription': return QQN::Courier()->ShortDescription;
				case 'ActiveFlag': return QQN::Courier()->ActiveFlag;
				default: throw new QCallerException('Simple Property not found in CourierDataGrid content: ' . $mixContent);
