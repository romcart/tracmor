<?php
	 ...
	 * listing a collection of Notification objects.  It includes
	 ...
	 * 
	 */
	class NotificationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Notification datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Notification object, or it can be any QQNode extending from QQN::Notification().
		 * 
		 ...
		 * sorting using two fields from the Notification object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Notification
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intNotificationId=<?=urlencode($_ITEM->NotificationId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->NotificationId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->NotificationId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Notification::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Notification, given the clauses above
			$this->DataSource = Notification::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Notification-based QQNode.
		 * It will also verify that it is a proper Notification-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notification') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notification".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'NotificationId': return QQN::Notification()->NotificationId;
				case 'ShortDescription': return QQN::Notification()->ShortDescription;
				case 'LongDescription': return QQN::Notification()->LongDescription;
				case 'Criteria': return QQN::Notification()->Criteria;
				case 'Frequency': return QQN::Notification()->Frequency;
				case 'EnabledFlag': return QQN::Notification()->EnabledFlag;
				case 'CreatedBy': return QQN::Notification()->CreatedBy;
				case 'CreatedByObject': return QQN::Notification()->CreatedByObject;
				case 'CreationDate': return QQN::Notification()->CreationDate;
				case 'ModifiedBy': return QQN::Notification()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Notification()->ModifiedByObject;
				case 'ModifiedDate': return QQN::Notification()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in NotificationDataGrid content: ' . $mixContent);
