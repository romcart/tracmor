<?php
	 ...
	 * listing a collection of AssetCustomFieldHelper objects.  It includes
	 ...
	 * 
	 */
	class AssetCustomFieldHelperDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this AssetCustomFieldHelper datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the AssetCustomFieldHelper object, or it can be any QQNode extending from QQN::AssetCustomFieldHelper().
		 * 
		 ...
		 * sorting using two fields from the AssetCustomFieldHelper object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from AssetCustomFieldHelper
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = AssetCustomFieldHelper::CountAll();
			}
			...
			$this->DataSource = AssetCustomFieldHelper::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a AssetCustomFieldHelper-based QQNode.
		 * It will also verify that it is a proper AssetCustomFieldHelper-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'asset_custom_field_helper') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "asset_custom_field_helper".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'AssetId': return QQN::AssetCustomFieldHelper()->AssetId;
				case 'Asset': return QQN::AssetCustomFieldHelper()->Asset;
				case 'Cfv1': return QQN::AssetCustomFieldHelper()->Cfv1;
				case 'Cfv3': return QQN::AssetCustomFieldHelper()->Cfv3;
				default: throw new QCallerException('Simple Property not found in AssetCustomFieldHelperDataGrid content: ' . $mixContent);
