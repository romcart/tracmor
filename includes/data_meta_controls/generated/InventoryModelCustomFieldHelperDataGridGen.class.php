<?php
	 ...
	 * listing a collection of InventoryModelCustomFieldHelper objects.  It includes
	 ...
	 * 
	 */
	class InventoryModelCustomFieldHelperDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this InventoryModelCustomFieldHelper datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the InventoryModelCustomFieldHelper object, or it can be any QQNode extending from QQN::InventoryModelCustomFieldHelper().
		 * 
		 ...
		 * sorting using two fields from the InventoryModelCustomFieldHelper object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from InventoryModelCustomFieldHelper
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = InventoryModelCustomFieldHelper::CountAll();
			}
			...
			// Set the DataSource to be a Query result from InventoryModelCustomFieldHelper, given the clauses above
			$this->DataSource = InventoryModelCustomFieldHelper::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a InventoryModelCustomFieldHelper-based QQNode.
		 * It will also verify that it is a proper InventoryModelCustomFieldHelper-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'inventory_model_custom_field_helper') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "inventory_model_custom_field_helper".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'InventoryModelId': return QQN::InventoryModelCustomFieldHelper()->InventoryModelId;
				case 'InventoryModel': return QQN::InventoryModelCustomFieldHelper()->InventoryModel;
				case 'Cfv2': return QQN::InventoryModelCustomFieldHelper()->Cfv2;
				case 'Cfv4': return QQN::InventoryModelCustomFieldHelper()->Cfv4;
				default: throw new QCallerException('Simple Property not found in InventoryModelCustomFieldHelperDataGrid content: ' . $mixContent);
