<?php
	 ...
	 * listing a collection of TransactionType objects.  It includes
	 ...
	 * 
	 */
	class TransactionTypeDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TransactionType datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TransactionType object, or it can be any QQNode extending from QQN::TransactionType().
		 * 
		 ...
		 * sorting using two fields from the TransactionType object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TransactionType
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intTransactionTypeId=<?=urlencode($_ITEM->TransactionTypeId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->TransactionTypeId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->TransactionTypeId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TransactionType::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TransactionType-based QQNode.
		 * It will also verify that it is a proper TransactionType-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'transaction_type') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "transaction_type".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'TransactionTypeId': return QQN::TransactionType()->TransactionTypeId;
				case 'ShortDescription': return QQN::TransactionType()->ShortDescription;
				case 'AssetFlag': return QQN::TransactionType()->AssetFlag;
				case 'InventoryFlag': return QQN::TransactionType()->InventoryFlag;
				default: throw new QCallerException('Simple Property not found in TransactionTypeDataGrid content: ' . $mixContent);
