<?php
	 ...
	 * listing a collection of Shortcut objects.  It includes
	 ...
	 * 
	 */
	class ShortcutDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Shortcut datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Shortcut object, or it can be any QQNode extending from QQN::Shortcut().
		 * 
		 ...
		 * sorting using two fields from the Shortcut object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Shortcut
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intShortcutId=<?=urlencode($_ITEM->ShortcutId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->ShortcutId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ShortcutId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Shortcut::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Shortcut-based QQNode.
		 * It will also verify that it is a proper Shortcut-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'shortcut') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "shortcut".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'ShortcutId': return QQN::Shortcut()->ShortcutId;
				case 'ModuleId': return QQN::Shortcut()->ModuleId;
				case 'Module': return QQN::Shortcut()->Module;
				case 'AuthorizationId': return QQN::Shortcut()->AuthorizationId;
				case 'Authorization': return QQN::Shortcut()->Authorization;
				case 'TransactionTypeId': return QQN::Shortcut()->TransactionTypeId;
				case 'TransactionType': return QQN::Shortcut()->TransactionType;
				case 'ShortDescription': return QQN::Shortcut()->ShortDescription;
				case 'Link': return QQN::Shortcut()->Link;
				case 'ImagePath': return QQN::Shortcut()->ImagePath;
				case 'EntityQtypeId': return QQN::Shortcut()->EntityQtypeId;
				case 'CreateFlag': return QQN::Shortcut()->CreateFlag;
				default: throw new QCallerException('Simple Property not found in ShortcutDataGrid content: ' . $mixContent);
