<?php
	 ...
	 * listing a collection of Audit objects.  It includes
	 ...
	 * 
	 */
	class AuditDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Audit datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Audit object, or it can be any QQNode extending from QQN::Audit().
		 * 
		 ...
		 * sorting using two fields from the Audit object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Audit
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAuditId=<?=urlencode($_ITEM->AuditId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AuditId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AuditId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Audit::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Audit, given the clauses above
			$this->DataSource = Audit::QueryArray($objCondition, $objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'AuditId': return QQN::Audit()->AuditId;
				case 'EntityQtypeId': return QQN::Audit()->EntityQtypeId;
				case 'CreatedBy': return QQN::Audit()->CreatedBy;
				case 'CreatedByObject': return QQN::Audit()->CreatedByObject;
				case 'CreationDate': return QQN::Audit()->CreationDate;
				case 'ModifiedBy': return QQN::Audit()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Audit()->ModifiedByObject;
				case 'ModifiedDate': return QQN::Audit()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in AuditDataGrid content: ' . $mixContent);
