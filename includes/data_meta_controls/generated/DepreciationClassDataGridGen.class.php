<?php
	 ...
	 * listing a collection of DepreciationClass objects.  It includes
	 ...
	 * 
	 */
	class DepreciationClassDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this DepreciationClass datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DepreciationClass object, or it can be any QQNode extending from QQN::DepreciationClass().
		 * 
		 ...
		 * sorting using two fields from the DepreciationClass object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DepreciationClass
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intDepreciationClassId=<?=urlencode($_ITEM->DepreciationClassId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->DepreciationClassId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->DepreciationClassId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = DepreciationClass::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DepreciationClass-based QQNode.
		 * It will also verify that it is a proper DepreciationClass-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'depreciation_class') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "depreciation_class".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'DepreciationClassId': return QQN::DepreciationClass()->DepreciationClassId;
				case 'DepreciationMethodQtypeId': return QQN::DepreciationClass()->DepreciationMethodQtypeId;
				case 'ShortDescription': return QQN::DepreciationClass()->ShortDescription;
				case 'Life': return QQN::DepreciationClass()->Life;
				default: throw new QCallerException('Simple Property not found in DepreciationClassDataGrid content: ' . $mixContent);
