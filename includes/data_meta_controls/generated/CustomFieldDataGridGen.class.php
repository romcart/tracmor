<?php
	 ...
	 * listing a collection of CustomField objects.  It includes
	 ...
	 * 
	 */
	class CustomFieldDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CustomField datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CustomField object, or it can be any QQNode extending from QQN::CustomField().
		 * 
		 ...
		 * sorting using two fields from the CustomField object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CustomField
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intCustomFieldId=<?=urlencode($_ITEM->CustomFieldId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->CustomFieldId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->CustomFieldId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CustomField::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CustomField, given the clauses above
			$this->DataSource = CustomField::QueryArray($objCondition, $objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'CustomFieldId': return QQN::CustomField()->CustomFieldId;
				case 'CustomFieldQtypeId': return QQN::CustomField()->CustomFieldQtypeId;
				case 'DefaultCustomFieldValueId': return QQN::CustomField()->DefaultCustomFieldValueId;
				case 'DefaultCustomFieldValue': return QQN::CustomField()->DefaultCustomFieldValue;
				case 'ShortDescription': return QQN::CustomField()->ShortDescription;
				case 'ActiveFlag': return QQN::CustomField()->ActiveFlag;
				case 'RequiredFlag': return QQN::CustomField()->RequiredFlag;
				case 'AllAssetModelsFlag': return QQN::CustomField()->AllAssetModelsFlag;
				case 'SearchableFlag': return QQN::CustomField()->SearchableFlag;
				case 'CreatedBy': return QQN::CustomField()->CreatedBy;
				case 'CreatedByObject': return QQN::CustomField()->CreatedByObject;
				case 'CreationDate': return QQN::CustomField()->CreationDate;
				case 'ModifiedBy': return QQN::CustomField()->ModifiedBy;
				case 'ModifiedByObject': return QQN::CustomField()->ModifiedByObject;
				case 'ModifiedDate': return QQN::CustomField()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in CustomFieldDataGrid content: ' . $mixContent);
