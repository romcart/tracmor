<?php
	 ...
	 * listing a collection of InventoryTransaction objects.  It includes
	 ...
	 * 
	 */
	class InventoryTransactionDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this InventoryTransaction datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the InventoryTransaction object, or it can be any QQNode extending from QQN::InventoryTransaction().
		 * 
		 ...
		 * sorting using two fields from the InventoryTransaction object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from InventoryTransaction
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intInventoryTransactionId=<?=urlencode($_ITEM->InventoryTransactionId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->InventoryTransactionId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->InventoryTransactionId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = InventoryTransaction::QueryCount($objCondition, $objClauses);
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'InventoryTransactionId': return QQN::InventoryTransaction()->InventoryTransactionId;
				case 'InventoryLocationId': return QQN::InventoryTransaction()->InventoryLocationId;
				case 'InventoryLocation': return QQN::InventoryTransaction()->InventoryLocation;
				case 'TransactionId': return QQN::InventoryTransaction()->TransactionId;
				case 'Transaction': return QQN::InventoryTransaction()->Transaction;
				case 'Quantity': return QQN::InventoryTransaction()->Quantity;
				case 'SourceLocationId': return QQN::InventoryTransaction()->SourceLocationId;
				case 'SourceLocation': return QQN::InventoryTransaction()->SourceLocation;
				case 'DestinationLocationId': return QQN::InventoryTransaction()->DestinationLocationId;
				case 'DestinationLocation': return QQN::InventoryTransaction()->DestinationLocation;
				case 'CreatedBy': return QQN::InventoryTransaction()->CreatedBy;
				case 'CreatedByObject': return QQN::InventoryTransaction()->CreatedByObject;
				case 'CreationDate': return QQN::InventoryTransaction()->CreationDate;
				case 'ModifiedBy': return QQN::InventoryTransaction()->ModifiedBy;
				case 'ModifiedByObject': return QQN::InventoryTransaction()->ModifiedByObject;
				case 'ModifiedDate': return QQN::InventoryTransaction()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in InventoryTransactionDataGrid content: ' . $mixContent);
