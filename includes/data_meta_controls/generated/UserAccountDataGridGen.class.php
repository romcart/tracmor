<?php
	 ...
	 * listing a collection of UserAccount objects.  It includes
	 ...
	 * 
	 */
	class UserAccountDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this UserAccount datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the UserAccount object, or it can be any QQNode extending from QQN::UserAccount().
		 * 
		 ...
		 * sorting using two fields from the UserAccount object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from UserAccount
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intUserAccountId=<?=urlencode($_ITEM->UserAccountId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->UserAccountId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->UserAccountId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = UserAccount::QueryCount($objCondition, $objClauses);
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'UserAccountId': return QQN::UserAccount()->UserAccountId;
				case 'FirstName': return QQN::UserAccount()->FirstName;
				case 'LastName': return QQN::UserAccount()->LastName;
				case 'Username': return QQN::UserAccount()->Username;
				case 'PasswordHash': return QQN::UserAccount()->PasswordHash;
				case 'EmailAddress': return QQN::UserAccount()->EmailAddress;
				case 'ActiveFlag': return QQN::UserAccount()->ActiveFlag;
				case 'AdminFlag': return QQN::UserAccount()->AdminFlag;
				case 'PortableAccessFlag': return QQN::UserAccount()->PortableAccessFlag;
				case 'PortableUserPin': return QQN::UserAccount()->PortableUserPin;
				case 'RoleId': return QQN::UserAccount()->RoleId;
				case 'Role': return QQN::UserAccount()->Role;
				case 'CreatedBy': return QQN::UserAccount()->CreatedBy;
				case 'CreatedByObject': return QQN::UserAccount()->CreatedByObject;
				case 'CreationDate': return QQN::UserAccount()->CreationDate;
				case 'ModifiedBy': return QQN::UserAccount()->ModifiedBy;
				case 'ModifiedByObject': return QQN::UserAccount()->ModifiedByObject;
				case 'ModifiedDate': return QQN::UserAccount()->ModifiedDate;
				default: throw new QCallerException('Simple Property not found in UserAccountDataGrid content: ' . $mixContent);
