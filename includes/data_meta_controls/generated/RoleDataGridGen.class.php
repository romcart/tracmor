<?php
	...
	class RoleDataGridGen extends QDataGrid {
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intRoleId=<?=urlencode($_ITEM->RoleId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->RoleId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->RoleId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Role::CountAll();
			}
			...
			$this->DataSource = Role::LoadAll($objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'RoleId': return QQN::Role()->RoleId;
				case 'ShortDescription': return QQN::Role()->ShortDescription;
				case 'LongDescription': return QQN::Role()->LongDescription;
				case 'CreatedBy': return QQN::Role()->CreatedBy;
				case 'CreatedByObject': return QQN::Role()->CreatedByObject;
				case 'CreationDate': return QQN::Role()->CreationDate;
				case 'ModifiedBy': return QQN::Role()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Role()->ModifiedByObject;
				case 'ModifiedDate': return QQN::Role()->ModifiedDate;
