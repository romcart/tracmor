<?php
	 ...
	 * listing a collection of AuthorizationLevel objects.  It includes
	 ...
	 * 
	 */
	class AuthorizationLevelDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this AuthorizationLevel datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the AuthorizationLevel object, or it can be any QQNode extending from QQN::AuthorizationLevel().
		 * 
		 ...
		 * sorting using two fields from the AuthorizationLevel object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from AuthorizationLevel
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAuthorizationLevelId=<?=urlencode($_ITEM->AuthorizationLevelId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AuthorizationLevelId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AuthorizationLevelId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = AuthorizationLevel::QueryCount($objCondition, $objClauses);
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'AuthorizationLevelId': return QQN::AuthorizationLevel()->AuthorizationLevelId;
				case 'ShortDescription': return QQN::AuthorizationLevel()->ShortDescription;
				default: throw new QCallerException('Simple Property not found in AuthorizationLevelDataGrid content: ' . $mixContent);
