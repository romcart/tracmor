<?php
	 ...
	 * listing a collection of CategoryCustomFieldHelper objects.  It includes
	 ...
	 * 
	 */
	class CategoryCustomFieldHelperDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CategoryCustomFieldHelper datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CategoryCustomFieldHelper object, or it can be any QQNode extending from QQN::CategoryCustomFieldHelper().
		 * 
		 ...
		 * sorting using two fields from the CategoryCustomFieldHelper object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CategoryCustomFieldHelper
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intCategoryId=<?=urlencode($_ITEM->CategoryId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->CategoryId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->CategoryId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CategoryCustomFieldHelper::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CategoryCustomFieldHelper-based QQNode.
		 * It will also verify that it is a proper CategoryCustomFieldHelper-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'category_custom_field_helper') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "category_custom_field_helper".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'CategoryId': return QQN::CategoryCustomFieldHelper()->CategoryId;
				case 'Category': return QQN::CategoryCustomFieldHelper()->Category;
				default: throw new QCallerException('Simple Property not found in CategoryCustomFieldHelperDataGrid content: ' . $mixContent);
