<?php
	 ...
	 * @property string $Version the value for strVersion (PK)
	...
	class VersionGen extends QBaseClass {
		...
		/**
		 * Protected member variable that maps to the database PK column _version.version
		 * @var string strVersion
		 */
		protected $strVersion;
		const VersionMaxLength = 50;
		 ...
		 * @var string __strVersion;
		 */
		protected $__strVersion;
		 ...
		 * @return Version
		 */
		public static function Load($strVersion) {
			// Use QuerySingle to Perform the Query
			return Version::QuerySingle(
				QQ::Equal(QQN::Version()->Version, $strVersion)
			);
		}
		...
		protected static function BuildQueryStatement(&$objQueryBuilder, QQCondition $objConditions, $objOptionalClauses, $mixParameterArray, $blnCountOnly) {
			...
			$objQueryBuilder = new QQueryBuilder($objDatabase, '_version');
			Version::GetSelectFields($objQueryBuilder);
			$objQueryBuilder->AddFromItem('_version');
		...
		/**
		 * Static Qcodo Query method to query for a single Version object.
		...
		public static function QuerySingle(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			...
			return Version::InstantiateDbRow($objDbResult->GetNextRow(), null, null, null, $objQueryBuilder->ColumnAliasArray);
		}

		/**
		 * Static Qcodo Query method to query for an array of Version objects.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		...
		/**
		 * Static Qcodo Query method to query for a count of Version objects.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
...
/*		public static function QueryArrayCached($strConditions, $mixParameterArray = null) {
			...
			$objCache = new QCache('query', '_version_' . serialize($strConditions));
			if (!($strQuery = $objCache->GetData())) {
				// Not Found -- Go ahead and Create/Build out a new QueryBuilder object with Version-specific fields
				$objQueryBuilder = new QQueryBuilder($objDatabase);
				Version::GetSelectFields($objQueryBuilder);
				Version::GetFromFields($objQueryBuilder);
		...
		*/
		public static function InstantiateDbRow($objDbRow, $strAliasPrefix = null, $strExpandAsArrayNodes = null, $objPreviousItem = null, $strColumnAliasArray = array()) {
			...
			$objToReturn->__strVersion = $objDbRow->GetColumn($strAliasName, 'VarChar');
		...
		public static function InstantiateDbResult(QDatabaseResultBase $objDbResult, $strExpandAsArrayNodes = null, $strColumnAliasArray = null) {
			...
			if ($strExpandAsArrayNodes) {
				...
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = Version::InstantiateDbRow($objDbRow, null, $strExpandAsArrayNodes, $objLastRowItem, $strColumnAliasArray);
					if ($objItem) {
						$objToReturn[] = $objItem;
		 ...
		 * @return Version
		*/
		public static function LoadByVersion($strVersion) {
			return Version::QuerySingle(
				QQ::Equal(QQN::Version()->Version, $strVersion)
			);
		}
		...
		public function Save($blnForceInsert = false, $blnForceUpdate = false) {
			...
			try {
				if ((!$this->__blnRestored) || ($blnForceInsert)) {
					...
					$objDatabase->NonQuery('
						...
						WHERE
							`version` = ' . $objDatabase->SqlVariable($this->__strVersion) . '
					');
				}
			...
			$this->__strVersion = $this->strVersion;
		...
		public function Reload() {
			...
			// Reload the Object
			$objReloaded = Version::Load($this->strVersion);
		...
		public function __get($strName) {
			switch ($strName) {
				...
				///////////////////
				case 'Version':
					// Gets the value for strVersion (PK)
					// @return string
		...
		public function __set($strName, $mixValue) {
			switch ($strName) {
				...
				///////////////////
				case 'Version':
					// Sets the value for strVersion (PK)
		...
		protected static function ArrayQueryHelper(&$strOrderBy, $strLimit, &$strLimitPrefix, &$strLimitSuffix, &$strExpandSelect, &$strExpandFrom, $objExpansionMap, &$objDatabase) {
			...
			if ($objExpansionMap) {
				$objQueryExpansion = new QQueryExpansion('Version', '_version', $objExpansionMap);
		...
		public static function ExpandQuery($strParentAlias, $strAlias, $objExpansionMap, QQueryExpansion $objQueryExpansion) {
			if ($strAlias) {
				$objQueryExpansion->AddFromItem(sprintf('LEFT JOIN `_version` AS `%s__%s` ON `%s`.`%s` = `%s__%s`.`version`', $strParentAlias, $strAlias, $strParentAlias, $strAlias, $strParentAlias, $strAlias));

				$objQueryExpansion->AddSelectItem(sprintf('`%s__%s`.`version` AS `%s__%s__version`', $strParentAlias, $strAlias, $strParentAlias, $strAlias));
	...
	class QQNodeVersion extends QQNode {
		protected $strTableName = '_version';
		protected $strPrimaryKey = 'version';
